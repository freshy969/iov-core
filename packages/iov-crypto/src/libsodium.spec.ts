...
import { Argon2id, Argon2idOptions, Ed25519, Ed25519Keypair, Random, Xchacha20poly1305Ietf, Xchacha20poly1305IetfCiphertext, Xchacha20poly1305IetfKey, Xchacha20poly1305IetfMessage, Xchacha20poly1305IetfNonce } from "./libsodium";
...
describe("Libsodium", () => {
  describe("Argon2id", () => {
    ...
    it("throw for invalid salt lengths", async () => {
      ...
      // 8 bytes
      await expect(Argon2id.execute(password, fromHex("aabbccddeeff0011"), options)).rejects.toThrow(/invalid salt length/);
      // 15 bytes
      await expect(Argon2id.execute(password, fromHex("aabbccddeeff001122334455667788"), options)).rejects.toThrow(/invalid salt length/);
      // 17 bytes
      await expect(Argon2id.execute(password, fromHex("aabbccddeeff00112233445566778899aa"), options)).rejects.toThrow(/invalid salt length/);
      // 32 bytes
      await expect(Argon2id.execute(password, fromHex("aabbccddeeff00112233445566778899aabbccddeeff00112233445566778899"), options)).rejects.toThrow(/invalid salt length/);
    });
  });
  ...
  describe("Ed25519", () => {
    ...
    it("generates keypairs", async () => {
...

      {
        ...
        await expect(Ed25519.makeKeypair(seed)).rejects.toThrow(/invalid seed length/i);
      }
  ...
  describe("Xchacha20poly1305Ietf", () => {
    ...
    it("throws when encrypting with wrong key length", async () => {
...

      {
        ...
        await expect(Xchacha20poly1305Ietf.encrypt(message, key, nonce)).rejects.toThrow(/invalid key length/i);
      }
      {
        // 31 bytes
    ...
    it("decryption fails with wrong ciphertext/key/nonce", async () => {
      ...
      }
      {
        ...
        await expect(Xchacha20poly1305Ietf.decrypt(corruptedCiphertext, key, nonce)).rejects.toThrow(/invalid usage/i);
      }
      {
        // corrupted key
        const corruptedKey = key.map((x, i) => (i === 0 ? x ^ 0x01 : x)) as Xchacha20poly1305IetfKey;
        await expect(Xchacha20poly1305Ietf.decrypt(ciphertext, corruptedKey, nonce)).rejects.toThrow(/invalid usage/i);
      }
      {
        // corrupted nonce
        const corruptedNonce = nonce.map((x, i) => (i === 0 ? x ^ 0x01 : x)) as Xchacha20poly1305IetfNonce;
        await expect(Xchacha20poly1305Ietf.decrypt(ciphertext, key, corruptedNonce)).rejects.toThrow(/invalid usage/i);
      }
    });
